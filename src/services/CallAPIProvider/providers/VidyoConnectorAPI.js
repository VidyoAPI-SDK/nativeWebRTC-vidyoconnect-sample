...
class VidyoConnectorAPI extends BaseCallAPIProvider {
      ...
      loadStyles(VidyoClientCSS)
        .then(() => {
          if (!window.appConfig.REACT_APP_DYNAMIC_VIDYOCLIENT) {
            return loadScript(VidyoClientJS, true);
          }
            ...
            this.vc
              .CreateVidyoConnector({
                ...
                remoteParticipants: OperatingSystemInfoProvider.IsTabletDevice()
                  ...
                  : participantLimit,
                logFileFilter:
                  "info@VidyoDevelopment info@VidyoClient info@VidyoSDP warning@VidyoTransport",
  ...
  setP2PConnection(options = {}) {
    return this.vidyoConnector.SetP2PConnection(options);
  }
        ...
        this.vidyoConnector
          .ConnectToRoomAsGuest(
            logCallbacks({
              ...
              displayName,
              roomPin,
              jwtToken, // optional, for now we use it only for call transfer
  ...
  getCallProperties() {
    return this.vidyoConnector.GetConnectionProperties().then((resultArray) => {
      ...
      return {
        ...
        conferenceId: result["Room.conferenceIdNumber"],
      };
    });
  }

  getJWTToken() {
    return this.vidyoConnector.GetJWTToken();
  }
  ...
  subscribeToTopic({
    ...
    if (typeof statusCallback !== "function") {
      statusCallback = (status, message, code) => {
        console.log(
          `Subscription to topic ${topicSubscriptionToken?.substr(
            -10
  ...
  unsubscribeFromTopic(topicSubscriptionToken) {
  ...
  unsubscribeFromParticipantsChanges() {
    this.vidyoConnector.ReportLocalParticipantOnJoined({
      reportLocalParticipant: false,
    });
    return this.vidyoConnector.UnregisterParticipantEventListener();
  }
  ...
  unsubscribeFromModerationEvents() {
    return this.vidyoConnector.UnregisterModerationCommandEventListener();
  }
      ...
      const logedAppState = `${new Date().toLocaleString()} | INFO | Application state: \n ${JSON.stringify(
        ...
        (key, value) => {
          ...
          switch (key) {
            case "message":
              if (value?.body) {
                newValue = { ...value, body: "value_hidden_for_logs" };
              }
              break;
            case "jwtToken":
  ...
  subscribeOnReconnectStateChange(onChange) {
    return this.vidyoConnector.RegisterReconnectEventListener(
      logCallbacks({
        onConferenceLost: (lastReason) => {
          onChange({
            event: "CONFERENCE_LOST",
            lastReason,
          });
        },
        onReconnected: () => {
  ...
  subscribeOnModerationTopic(onModerationTopicMessage) {
    return this.vidyoConnector.RegisterModerationTopicMessageEventListener({
      onModerationTopicMessage,
    });
  }

  async subscribeOnBreakoutRooms(
    onTransferring,
    onTransferred,
    onTransferFailed
  ) {
    return this.vidyoConnector.RegisterBreakoutRoomsEventListener(
      logCallbacks({
  ...
  async subscribeOnBroadcastMessages(onBroadcastMessage) {
    return this.vidyoConnector.RegisterBroadcastMessageEventListener(
      logCallbacks({
  ...
  raiseHand(callback, requestId = "") {
    return this.vidyoConnector.RaiseHand(
      logCallbacks({
        raiseHandResponse: callback,
        requestId,
      })
    );
  }
  ...
  unraiseHand(requestId = "") {
    return this.vidyoConnector.UnraiseHand({
      requestId,
    });
  }

  async exitBreakoutRoom() {
    return this.vidyoConnector.ExitBreakoutRoom();
  }

  async unsubscribeFromBreakoutRooms() {
    return this.vidyoConnector.UnregisterBreakoutRoomsEventListener();
  }

  async unsubscribeFromBroadcastMessages() {
    return this.vidyoConnector.UnregisterBroadcastMessageEventListener();
  }

  unsubscribeFromModerationTopic() {
    return this.vidyoConnector.UnregisterModerationTopicMessageEventListener();
  }

  unsubscribeFromRemoteCamerasChanges() {
    return this.vidyoConnector.UnregisterRemoteCameraEventListener();
  }

  unsubscribeFromRemoteMicrophonesChanges() {
    return this.vidyoConnector.UnregisterRemoteMicrophoneEventListener();
  }

  unsubscribeFromCompositorViewChanges() {
    return this.vidyoConnector.UnregisterCompositorViewChangeEventListener();
  }

  unsubscribeFromCameraPresetsChanges() {
    return this.vidyoConnector.UnregisterRemoteCameraPresetChangeEventListener();
  }

  unsubscribeFromCameraControlPanelChanges() {
    return this.vidyoConnector.UnregisterCameraControlsPanelChangeEventListener();
  }
}
