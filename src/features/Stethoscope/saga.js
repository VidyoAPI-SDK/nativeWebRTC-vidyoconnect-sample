...
// it is just for nativeWebrtc react sample
function* handleLocalCameras(action) {
  for(const camera of action.cameras) {
    try {
      yield camera.AllowRemoteCameraControl?.({ allow: true });
    } catch (e) {
      //don't care
    }
  }
...
function* actionWatcher() {
  ...
  yield takeLatest(
    devicesActionType.UPDATE_CAMERAS,
    handleLocalCameras
  );
  yield takeLatest(
