...
const AccessCode = ({
  ...
  useEffect(() => {
    if (isPinTooltipOpened) {
      setTimeout(
        () => document.getElementById("accessCodeErrorTooltip")?.focus(),
        300
      );
    }
  }, [isPinTooltipOpened]);
    ...
    <div className="access-code-container">
      <div className="header">
        <p>
          ...
          <span tabIndex="0">{t("ACCESS_CODE_REQUIRED_TO_JOIN")}</span>
        </p>
      </div>
      <div className="controls">
        <Tooltip
          ...
          content={
            <span
              id="accessCodeErrorTooltip"
              {...test("ACCESS_CODE_ERROR_MESSAGE")}
            >
        ...
        >
          <BeautyInput
            ...
            aria-labelledby="accessCodeErrorTooltip"
            aria-invalid={isPinTooltipOpened ? "true" : "false"}
            autoFocus
          />
        </Tooltip>
        <p tabIndex="0" className="guest-bottom-text">
        ...
        <Button
          {...test("JOIN_BUTTON")}
          fill={true}
          data-invalid={!accessCode}
          aria-disabled={!accessCode ? "true" : "false"}
